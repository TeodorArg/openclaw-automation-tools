...
- do not trust a copied local plugin directory to have a fresh built `dist/**` just because `package.json` still shows the expected version; if runtime code changed after the last release or local copy, rebuild the package before the final install and prefer a forced reinstall or fresh tarball install so the gateway does not keep serving a stale built artifact under the same version string
...
- when the same package version may already be present in the gateway, prefer `openclaw plugins install --force ...` for the final install step so the runtime cannot keep an older linked or copied build under the same version label
